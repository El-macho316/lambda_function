...
def lambda_handler(event, context):
    try:
        ...
        if not test_yfinance_connection():
            logger.error("yfinance connection test failed")
            return {
                "statusCode": 503,
                "body": json.dumps({
                    "error": "ServiceUnavailable",
                    "message": "Unable to connect to financial data service. Please try again later."
                })
            }
        ...
        if not data:
            logger.error(f"No financial data retrieved for {ticker}")
            return {
                "statusCode": 404,
                "body": json.dumps({
                    "error": "TickerNotFound",
                    "message": f"Financial data for ticker '{ticker}' could not be retrieved. Please verify the ticker symbol is correct."
                })
            }
        ...
        if not data.get("companyName"):
            ...
            return {
                ...
                "body": json.dumps({
                    "error": "InvalidTicker",
                    "message": f"Ticker '{ticker}' does not appear to be a valid stock symbol."
                })
            }
